...
class MINIMDA(nn.Module):
    ...
    def forward(self, md_graph, miRNA, disease):

        emb_ass = self.gcn_md(md_graph, th.cat((miRNA, disease), dim=0))
        emb_mm = emb_ass[:self.args.miRNA_number, :]
        emb_dd = emb_ass[self.args.miRNA_number:, :]

        return emb_mm, emb_dd
